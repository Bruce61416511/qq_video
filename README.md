...
│  │ LLM 分镜 │  │ TTS 配音 │  │ 视频生成 │           │
│  │ DeepSeek │  │ 百炼     │  │ Wan-2.1  │           │
│  │ OpenAI   │  │ CosyVoice│  │ Kling    │           │
│  │ 通义千问 │  │ OpenAI   │  │ 即梦     │           │
│  │ 智谱 GLM │  │ ChatTTS  │  │ Runway   │           │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘           │
│       └──────────────┴─────────────┘                 │
│                      ↓                               │
│          ffmpeg 合成（画面 + 配音 + 字幕）             │
...
│   │   └── services/
│   │       ├── llm_service.py     # LLM 分镜（DeepSeek/OpenAI/通义千问等）
...
进入 **设置** → 配置三个环节（可按需只配部分）：

| 环节 | 推荐服务 | 申请地址 |
|------|---------|---------|
| LLM 分镜 | DeepSeek（最便宜） | https://platform.deepseek.com/api_keys |
| TTS 配音 | 阿里云百炼 CosyVoice | https://bailian.console.aliyun.com |
| 视频生成 | 阿里云百炼 Wan-2.1 | https://bailian.console.aliyun.com |

> TTS 和视频生成可共用同一个百炼 API Key。
...
| 环节 | 单镜耗时 | 说明 |
|------|---------|------|
| TTS 配音 | 1-3 秒 | 百炼 CosyVoice，WebSocket 流式 |
...
| 服务 | 模型 | 说明 |
|------|------|------|
| 百炼 CosyVoice | qwen-audio-3.0-tts-flash | 国内直连，WebSocket 流式，音色可选 |
